...
class iouEval:
    def __init__(self, n_classes, device, ignore=None):
        ...
        print("[IOU EVAL] IGNORE: ", self.ignore)  # tensor([0])
        print("[IOU EVAL] INCLUDE: ", self.include)  # [IOU EVAL] INCLUDE:  tensor([1, 2])
        self.reset()
    ...
    def getRecall(self):
        ...
        recall_mean = tp[self.include].sum() / (tp[self.include].sum() + fn[self.include].sum() + 1e-15)
        return recall_mean, recall
    
    def getPrecision(self):
        tp, fp, fn = self.getStats()
        precision = tp / (tp + fp + 1e-15)
        precision_mean = tp[self.include].sum() / (tp[self.include].sum() + fp[self.include].sum() + 1e-15)
