...
from modules.MotionRV import *

from modules.PointRefine.spvcnn import SPVCNN
from modules.PointRefine.spvcnn_lite import SPVCNN

class User():
    def __init__(self, ARCH, DATA, datadir, outputdir, modeldir, split, model_i=None, point_refine=False):
        # parameters
        self.ARCH = ARCH
        self.DATA = DATA
        ...
        self.parser = parserModule.Parser(root=self.datadir,
                                          ...
                                          workers=self.ARCH["train"]["workers"], # self.ARCH["train"]["workers"],
                                          gt=True,
                                          shuffle_train=False)
        ...
        with torch.no_grad():
            ...
            if not point_refine:
                self.model = MotionRV(self.parser.get_n_classes(), ARCH, num_batch=self.infer_batch_size)
                ...
                if model_i is None:                    
                    checkpoint = f"MotionRV_valid_best"
                else:
                    checkpoint = f"MotionRV_valid_best_{model_i}"
                ...
                self.model = MotionRV(self.parser.get_n_classes(), ARCH, num_batch=self.infer_batch_size)
                self.model = nn.DataParallel(self.model)
                checkpoint = "MotionRV_refine_module_valid_best"
                w_dict = torch.load(f"{self.modeldir}/{checkpoint}", map_location=lambda storage, loc: storage)
