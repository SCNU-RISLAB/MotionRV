...
class LaserScan:
    ...
    def __init__(self, project=False, H=64, W=1024, fov_up=3.0, fov_down=-25.0, DA=False,
                 flip_sign=False, rot=False, drop_points=False, use_normal=False, knn=False):
    ...
    def set_points(self, points, remissions=None):
        ...
        # if knns is not None:
        #     self.knns = knns  # get remission
        #     #if self.DA:
        #     #    self.remissions = self.remissions[::-1].copy()
        # else:
        #     self.knns = np.zeros((points.shape[0]), dtype=np.float32)            
    ...
    def do_range_projection(self):
        ...
        indices = np.arange(depth.shape[0])  # 从小到大排序后，提取对应的索引index
...
class SemLaserScan(LaserScan):
    ...
    def __init__(self, sem_color_dict=None, project=False,
                 ...
                 DA=False, flip_sign=False, drop_points=False, use_normal=False, knn=False):
        super(SemLaserScan, self).__init__(project, H, W, fov_up, fov_down, DA=DA,
                                           flip_sign=flip_sign, drop_points=drop_points, use_normal=use_normal, knn=knn)
        self.reset()
